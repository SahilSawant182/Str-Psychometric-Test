"""
setup_skill_career_path_quota.py
================================
One-time setup script: registers the SKILL_CAREER_PATH feature in Application Features
...
Run via bench execute:
    bench --site devstridenex.quantcloud.in execute \
        nexedu.path_finder.setup_skill_career_path_quota.run
"""
...
FEATURE_CODE  = "SKILL_CAREER_PATH"
FEATURE_TITLE = "Skill Career Path"    # human-readable name shown in UI
FEATURE_APP   = "stridenex_app"        # options: stridenex_app, etc.
...
def run():
    ...
    for acc in accounts:
        try:
            ...
            frappe.log_error(
                title=f"setup_skill_career_path_quota – BAM re-allocate error: {acc.name}",
                message=frappe.get_traceback()
            )

    print("\n✅ Setup complete. SKILL_CAREER_PATH quota is now active.")
